...
const JWT_SECRET  = process.env.JWT_SECRET || 'edupulse_dev_secret'
...
function dbErr(error) { throw new Error(error?.message || 'Database error') }
...
export async function registerUser({ name, email, password }) {
  ...
  const db = getDb()
  const { data: existing } = await db.from('users').select('id').eq('email', email.trim().toLowerCase()).maybeSingle()
  if (existing) throw new Error('An account with this email already exists.')
  const hashed = await hashPassword(password)
  const { data: user, error } = await db.from('users').insert({ name: name.trim(), email: email.trim().toLowerCase(), password: hashed }).select().single()
...
export async function loginUser({ email, password }) {
  const db = getDb()
  const { data: user, error } = await db.from('users').select('*').eq('email', email?.trim().toLowerCase() || '').maybeSingle()
  if (error) dbErr(error)
...
export async function updateUserProfile(id, { name, university, bio, avatar }) {
  ...
  const { data: user, error } = await db.from('users').update(updates).eq('id', id).select().single()
  if (error) dbErr(error)
  return safeUser(user)
}
...
export async function updateUserPassword(id, { currentPassword, newPassword }) {
  ...
  const { error: updateError } = await db.from('users').update({ password: hashed }).eq('id', id)
...
export async function updateUserSettings(userId, settings) {
  ...
  const updates = {}
  for (const key of allowed) { if (key in settings) updates[key] = Boolean(settings[key]) }
  if (!Object.keys(updates).length) return getUserSettings(userId)
  const { data, error } = await db.from('user_settings').update(updates).eq('user_id', userId).select().single()
  if (error) dbErr(error)
  return data
}
